...
interface ImportEventbriteEventFormProps {
    open: boolean;
    onClose: () => void;
    onImport: (eventBriteEventUrl: string) => Promise<void>;
}

export default function EventbriteURLInputForm({
                                                 open,
                                                 onClose,
                                                 onImport,
                                             }: ImportEventbriteEventFormProps) {
    const [eventbriteEventUrl, setEventbriteEventUrl] = useState("");
    const [loading, setLoading] = useState(false);

    if (!open) return null;

    const handleImport = async () => {
        if (!eventbriteEventUrl.trim()) return;

        try {
            setLoading(true);
            await onImport(eventbriteEventUrl.trim());
            setEventbriteEventUrl("");
            onClose();
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="modal-overlay">
            <div className="modal">
                ...
                <h2>Import Eventbrite Event</h2>

                <p style={{marginBottom: "16px"}}>
                    Paste your Eventbrite URL to import event details
                </p>
                ...
                <input
                    ...
                    value={eventbriteEventUrl}
                    onChange={(e) => setEventbriteEventUrl(e.target.value)}
                ...
                >
                    ...
                    <button className="btn btn-primary-greater"
                            ...
                            disabled={loading || !eventbriteEventUrl.trim()}
                    >
