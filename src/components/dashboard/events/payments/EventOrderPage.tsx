...
export default function EventOrderPage({
    ...
    return (
        <div>
            <h3>Event Order</h3>
            <p>Event ID: <b>{eventOrder.event_id}</b></p>
            <p>Service Type: <b>{eventOrder.promote_selection}</b></p>

            <h3>Payment</h3>
            <p>Provider:&nbsp;
                <svg width="60" height="24" viewBox="0 0 360 150" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path fill-rule="evenodd" clip-rule="evenodd"
                          ...
                          fill="#533AFD"/>
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M169.8 25.7L194.9 20.3V0L169.8 5.3V25.7Z"
                          fill="#533AFD"/>
            ...
            <p>Amount: <b>{eventOrderManager.getOrderCost(eventOrder)}</b></p>
            <div style={{display: 'flex', flexDirection: "row", padding: "6px", justifyContent: "center"}}>
                {!orderPaid &&
                    <>
                        <button onClick={() => onMakePayment(eventOrder)} className="btn btn-primary">Make a
                             Payment
                        </button>
                        <button onClick={() => onPaymentIncomplete()} className="btn btn-secondary">Cancel</button>
                    </>
                }
                {orderPaid &&
                    <>Payment OK &nbsp;
                        <button onClick={() => onPaymentComplete()} className="btn btn-secondary">Done</button>
                    </>
                }
            </div>
        </div>
